...
class BridgeObjectiveFunction(ObjectiveFunction):
  def __init__(self, is_minimization: bool, model, train_loader, validation_loader, device_to_use, proportion_rate):
  ...
  def __compute_fitness(self, mask):
    ...
    pruned_model = copy.deepcopy(self.__model).to(self.__device_to_use)
    self.__apply_pruning(pruned_model, mask)

    return self.__evaluate_mask(pruned_model)

  def __evaluate_mask(self, model):
